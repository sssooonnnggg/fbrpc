...
namespace fbrpc
{
	...
	std::unique_ptr<sScope> sCppPrinter::addNamespace(std::string_view name)
	{
		addSingleLine(std::string("namespace ") + std::string(name) + " ");
		return addScope();
	}

	std::unique_ptr<sScope> sCppPrinter::addClass(std::string_view className, std::string_view baseClassName)
	{
	...
	std::unique_ptr<sScope> sCppPrinter::addScope()
	{
		return std::make_unique<sScope>(this, [this]() {addSingleLine("{"); }, [this]() {addSingleLine("}"); });
	}
}